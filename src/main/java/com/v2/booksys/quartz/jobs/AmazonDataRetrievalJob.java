package com.v2.booksys.quartz.jobs;
import java.io.File;
...
public class AmazonDataRetrievalJob implements Job {
	org.slf4j.Logger logger = LoggerFactory.getLogger(AmazonDataRetrievalJob.class);
	private  final String SECRET_KEY = "4QhAQO5rDHqjYQRDjvgDg/GR7r038zpX3d2ZngvW";
    private  final String AWS_KEY = "AKIAIHYXAS2JB4ND2N6A";
    private  final String ASSOCIATE_TAG = "academ058-20";	
    SignedRequestsHelper helper;
	
    public void execute(JobExecutionContext context)
            throws JobExecutionException {
    	PropertyConfigurator.configure("log4j_Jobs.xml");
    	logger.info("Executing the sxheduled job at the scheduled time");	
    	System.out.println("s.o.p Executing the sxheduled job at the scheduled time");
		...
		try {
			...
			e.printStackTrace();
			logger.error("Citical error. Can't get SignedRequestsHelper instance", e);
			return;
		}
       ...
       // params.put("Keyword", "Test");
		...
		try {
			...
			// TODO Auto-generated catch block
			e.printStackTrace();
			logger.error("Citical error. Can't read the key words file", e);
			return;
		}
    ...
    private static List<Book> convertPageResultsIntoBooks(Node nodeItems, Map map, BookService bookService, String keyword){
  ...
  private  Document retrieveDocumentResponse(String url, Map map) throws DocumentException{
